...
//    }
    ...
    public void getAllMilkTea(HttpServletResponse response){
        ...
        List<MilkTeaForExcel> MilkTeaForExcel= milkteaService.selectMilkTeaForExcel();
        SimpleDateFormat sdf=new SimpleDateFormat("yyyyMMdd");
        String time=sdf.format(new Date());
        time=time.replaceAll("-","").replaceAll(":","").replaceAll(" ","");
        String fileName="全部奶茶信息";
        ExcelUtil.writeExcel(response,MilkTeaForExcel,fileName+time,fileName,new MilkTeaForExcel());
    }
